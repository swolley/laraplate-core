...
For operator-triggered bulk imports, Core provides `AbstractImportCommand`, `BulkImporterInterface`, module-injected resolver/discovery contracts, `ContainerBulkImporterResolver`, `FilesystemImportPluginDiscovery`, and `BulkImportRunner`. Core intentionally registers no `core:import`: concrete modules expose destination-specific commands and marker interfaces. Shared options are inherited through `getOptions()` (`--importer`, `--bootstrap`, repeatable `--arg`, `--dry-run`, `--limit`, `--no-search`). Dry-run rolls back only the default database connection; importers must suppress files, queues, HTTP calls, other connections, and other non-transactional side effects. Continuous synchronization remains a separate design requiring cursors, identities, conflicts, retries, and scheduling.
