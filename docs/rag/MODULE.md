...
Authorization **reuses the owner entity's CRUD permissions** — no media-specific permissions exist: LIST requires the entity's `select`; a bound UPLOAD (with id) and DELETE require the entity's `update`; an id-less (pending) UPLOAD requires the entity's `insert`. Enforced through `AuthorizationService::ensurePermission()` against the owner model's table + connection (same check as the CRUD pipeline). An `AuthorizationException` maps to a 401 envelope like sibling CRUD endpoints; an entity whose model is not media-enabled returns 404. `MediaResource` serializes `id`, `uuid`, `collection_name`, `name`, `file_name`, `mime_type`, `size`, `order_column`, `custom_properties`, `url`, a `conversions` map (generated conversion name → url) derived generically from the media's own generated conversions, and `draft_token` (the owning draft's token while the asset is still staged, else `null`).
