...
**Pending bucket + claim (CREATE forms).** During a create form the owner record has no id yet, so uploads bind to a token-keyed bucket (`Core\Models\MediaDraft`, table `core_media_drafts`, one media collection `pending`) and are later moved onto the freshly created record. All pending routes live in the same `/crud` group under `core.crud.media.*`:

-   `POST /app/crud/media/pending/{module}/{entity}` (`pending.upload`): auth = the entity's **`insert`** permission. Body: `file` (required), `collection` (required, validated against the *target* model's registered collections → 422 if unknown), `token` (required uuid), optional `name`, optional `custom_properties[]`. Resolves/creates the `MediaDraft` for `(auth user, token, module, entity)` via `firstOrCreate`, stores the file in the `pending` collection with the intended `target_collection` recorded as a custom property, and returns a `MediaResource` with `201` (`collection_name` is `pending`).
